...
#
# Author: Trevor Ablett, Jonathan Kelly, Yinjie Lu
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    Kk = p_cov_check @ h_jac.T @ np.linalg.inv(h_jac @ p_cov_check @ h_jac.T + sensor_var*np.eye(3))
    # 3.2 Compute error state
    delta_x = Kk @ (y_k - p_check).reshape([3,1])
    # 3.3 Correct predicted state
    p_hat = p_check + delta_x[0:3, 0]
    v_hat = v_check + delta_x[3:6, 0]
    q_hat = Quaternion(w=q_check[0],x=q_check[1],y=q_check[2],z=q_check[3]).quat_mult_right(Quaternion(axis_angle=angle_normalize(delta_x[6:, 0])))
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_check = p_est[k-1] + delta_t*v_est[k-1] + delta_t**2/2*(Cns @ imu_f.data[k-1].reshape(3,1) + g.reshape(3,1)).reshape(3)
    v_check = v_est[k-1] + delta_t*(Cns @ imu_f.data[k-1].reshape(3,1) + g.reshape(3,1)).reshape(3)
    q_check = Quaternion(*q_est[k-1]).quat_mult_left(Quaternion(axis_angle=angle_normalize(delta_t*imu_w.data[k-1])))

    # 1.1 Linearize the motion model and compute Jacobians
    f_jac = np.zeros([9, 9])
    f_jac[0:3,0:3] = np.eye(3)
    f_jac[3:6,3:6] = np.eye(3)
    f_jac[6:9,6:9] = np.eye(3)
    f_jac[0:3,3:6] = delta_t*np.eye(3)
    f_jac[3:6,6:9] = -delta_t*skew_symmetric((Cns @ imu_f.data[k-1].reshape(3,1)).reshape(3))
    Q = np.zeros([6, 6])
    ...
    gnssmax = len(gnss.t)
    lidarmax = len(lidar.t)
    if gnss_i < gnssmax and lidar_i < lidarmax and imu_f.t[k-1] == gnss.t[gnss_i] and imu_f.t[k-1] == lidar.t[lidar_i]:
        ...
        gnss_i += 1
        lidar_i += 1
    elif gnss_i < gnssmax and imu_f.t[k-1] == gnss.t[gnss_i]:
        y_k = gnss.data[gnss_i]
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov_check, y_k, p_check, v_check, q_check)
        gnss_i += 1
    elif lidar_i < lidarmax and imu_f.t[k-1] == lidar.t[lidar_i]:
        y_k = lidar.data[lidar_i]
